...
- I've also conducted a quick simulation on this matter, and the results appear unbiased. Please refer to [the Mplus output](https://github.com/sooyongl/customLMS/blob/main/monte_binary.out) for details.
...
item_2 = \lambda_{1} \eta_1 + \epsilon_{item_2} \\
item_3 = \lambda_{1} \eta_1 + \epsilon_{item_3} \\
...
Note that $c_1$ cannot be directly estimated; thus, $c_1$ will be replaced with Hermite-Gaussian quadrature later in the procedure.
...
-   $\nu_{z}$ and $\nu_{x_1}$ are fixed at the means of $z$ and $x_1$ (no need for estimating).
...
$$ 
\Sigma\_{yy}(c1) = \Lambda*y( T I_{\text{block}} T')\Lambda\_y' +
\Lambda*y*\Psi{y}\Lambda\_y' + \theta\_y
$$


The above model-implied $\mu$ and $\Sigma$ are fed into the multivariate normal density, $\phi$. The density value for the realizaiton (x, y) across k-dimensional real space $R^k$ is

$$
f(x, y) = \int_{R^k} \phi(c_1)\phi_{\mu(c_1),\Sigma(c_1)} (x,y)  \,dc_1
$$
...
Because of the nonlinear relationship mentioned above, the integral of the mixture density
...
$$
f(x, y) = \int_{R^k} \pi^{-m/2}exp(-u'u)\phi_{\mu(2^{-1/2}c_1),\Sigma(2^{-1/2}c_1)} (x,y)  \,du
$$
...
Using latent response variable framework (Muthen & Asp, 2002), $item^*$ is used instead of $item$ for multivariate normal distribution with some more constraints (i.g., $E(item^*)=0$ and $V(item^*)=1$. The relationship between binary indicators and other variables is estimated by tetrachoric correlation, polyserial correlation (point-biserial in our case). 
