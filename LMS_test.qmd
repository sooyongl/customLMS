...
The binary variable like treatment assignments is treated as continuous variable like in a typical regression model with dummy variable.
...
$$
\Sigma_{xx}(c_1) = 
\Lambda_x A I_block A' \Lambda_x' + \Theta
$$
...
$$
\theta^{(3)} = arg \max_\theta\{\sum_{i=1}^N \sum_{j=1}^{M} p^{(r)}(j=j|x_i,y_i)ln\phi_{\mu_{c_1},\Sigma_{c_1}})(x_i, y_i)\}
$$
